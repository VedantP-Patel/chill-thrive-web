...
export default function AdminPage() {
  ...
  const [filteredBookings, setFilteredBookings] = useState<Booking[]>([]); // <--- ADD THIS LINE
...
// --- 4. SMART FILTERING LOGIC (FIXED) ---
  ...
  }, [bookings, searchTerm, bookingFilter]);
...
const handleMarkDone = async (id: string) => {
    ...
    if (!error) {
        handleRefresh();
        // Optional: Switch to History or stay on Active
    }
  };
...
const handleMarkActive = async (id: string) => {
    // 1. Update DB to 'in_progress'
    const { error } = await supabase.from("bookings").update({ status: 'in_progress' }).eq("id", id);
    
    if (error) {
        alert("Error starting session: " + error.message);
    } else {
        // 2. Refresh list immediately
        handleRefresh();
        // 3. Switch tab to Active so user sees the change
        setBookingFilter('active'); 
    }
  };

  // 2. Mark as No Show (Treats as Cancelled/Ghosted)
  const handleNoShow = async (id: string) => {
    if(!confirm("Mark this user as 'No Show'? This will cancel the booking.")) return;
...
{/* TAB 1: BOOKINGS PAGE (V8.0 - WITH ACTIVE QUEUE) */}
        {activeTab === "bookings" && (
            <div className="w-full h-full overflow-y-auto bg-slate-50/50 relative">
              ...
              <div className="max-w-7xl mx-auto p-6 md:p-10 space-y-8 relative z-10">
                ...
                {/* Filter Nav (NOW INCLUDES 'active') */}
                <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
                  <div className="flex bg-slate-100 p-1.5 rounded-2xl w-full md:w-auto overflow-x-auto">
                    {['active', 'pending', 'upcoming', 'history'].map((tab) => (
                      <button
                        key={tab}
                        type="button"
                        onClick={() => setBookingFilter(tab as any)}
                        className={`flex-1 md:flex-none px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                          bookingFilter === tab 
                          ? (tab === 'active' ? 'bg-amber-400 text-black shadow-md' : 'bg-white text-black shadow-md') 
                          : 'text-slate-400 hover:text-slate-600'
                        }`}
                      >
                        {tab === 'active' && <span className="animate-pulse mr-1">●</span>} {tab}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex flex-col gap-3">
                  {filteredBookings.length > 0 ? (
                    filteredBookings.map((b) => {
                      ...
                      const isUnresolved = !['done', 'cancelled', 'rejected', 'completed', 'in_progress'].includes((b.status || '').toLowerCase());
                      const isOverdue = isPast && isUnresolved;
                      const isActive = (b.status || '').toLowerCase() === 'in_progress';
                      ...
                      return (
                        <div 
                          ...
                          className={`border rounded-xl p-4 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row items-center gap-4 group relative overflow-hidden ${
                            isActive ? 'bg-amber-50 border-amber-200 ring-2 ring-amber-100' :
                            isOverdue ? 'bg-red-50/40 border-red-300 ring-1 ring-red-100' : 
                            'bg-white border-slate-200'
                          }`}
                        >
                          {/* Live Indicator Strip */}
                          {isActive && <div className="absolute top-0 left-0 w-1.5 h-full bg-amber-400 animate-pulse"></div>}
                          {isOverdue && !isActive && <div className="absolute top-0 left-0 w-1 h-full bg-red-400"></div>}
                          
                          {/* Date Capsule */}
                          <div className={`flex flex-col items-center justify-center w-14 h-14 rounded-lg shrink-0 text-white shadow-lg ${isActive ? 'bg-amber-500 shadow-amber-200' : isOverdue ? 'bg-red-500 shadow-red-200' : 'bg-slate-900 shadow-slate-200'}`}>
                          ...
                          <div className="flex-1 min-w-[180px] text-center md:text-left">
                            <div className="flex items-center justify-center md:justify-start gap-2">
                              ...
                              {isActive && <span className="px-2 py-0.5 bg-amber-100 text-amber-700 text-[9px] font-black uppercase rounded-full animate-pulse">⚡ Live Session</span>}
                              {isOverdue && !isActive && <span className="px-2 py-0.5 bg-red-100 text-red-600 text-[9px] font-black uppercase rounded-full">⚠ Overdue</span>}
                            </div>
                          ...
                          <div className="md:w-48 hidden md:block border-l border-slate-200/50 pl-4">
                            ...
                            <p className="text-[10px] font-mono mt-0.5 text-slate-500">🕒 {b.time} ({b.duration})</p>
                          </div>

                          {/* Financials */}
                          <div className="md:w-40 text-right px-3 py-2 rounded-lg border bg-slate-50 border-slate-100 min-w-[140px] flex flex-col justify-center">
                            ...
                            <div className="text-[10px] font-mono text-slate-500 truncate mt-0.5" title={b.transaction_id}>{b.transaction_id || 'N/A'}</div>
                          </div>
                          ...
                          <div className="flex gap-2 w-full md:w-auto mt-3 md:mt-0 justify-end md:pl-4 md:border-l md:border-slate-200/50 min-w-fit">
                            ...
                            {bookingFilter === 'upcoming' && (
                              <div className="flex gap-2">
                                ...
                                {/* MOVE BUTTON */}
                                <button type="button" onClick={(e) => { e.stopPropagation(); openReschedule(b); }} className="px-3 py-2 bg-slate-100 text-slate-700 font-bold text-[10px] uppercase rounded-lg hover:bg-slate-200">
                                    Move
                                </button>
                            ...
                            {bookingFilter === 'history' && (
                               <span className={`px-3 py-1.5 rounded-md text-[10px] font-black uppercase border ${(b.status||'').toLowerCase().includes('done') || (b.status||'').toLowerCase().includes('complete') ? 'bg-green-50 text-green-600 border-green-100' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
